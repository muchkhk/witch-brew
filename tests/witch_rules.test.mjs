...
const TAKEOVER_REQ = { seat: 1, name: "Newcomer", ts: 1 };
...
test("不正な申請データ（範囲外の席・余分なフィールド）を拒否する", async () => {
  await assertFails(room(db(NEWUID), `takeoverRequests/${NEWUID}`).set({ seat: 9, name: "X", ts: 1 }));
  await assertFails(room(db(NEWUID), `takeoverRequests/${NEWUID}`).set({ seat: 1, name: "X", ts: 1, evil: true }));
});
