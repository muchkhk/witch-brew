#!/usr/bin/env python3
"""
思考課金ポーカー v0-A モンテカルロ検証シミュレータ

指示書: 思考課金ポーカー v0-A モンテカルロ検証 v1
...
KNOBS = {
    ...
    "raise_amount": 2,
    "raise_cap": 4,
    "peek_cost": 2,  # 将来案: max(2, ceil(pot*0.25)) だが今回は固定2のみ実装
    "max_rounds": 30,
}
...
CALL_MARGIN = 0.05  # ポットオッズ「マージン」の値。指示書に明記が無いため独自設定 (報告事項に記載)
SIGNAL_ADJUST = 0.05  # S7 のシグナル読み取りによる要求勝率の緩和/厳格化幅
...
class DecisionContext:
    ...
    def __init__(self, rng, me, opp, me_pool, opp_pool, pot, call_amount,
                 bet_level, raises_used, round_history, facing_bet):
        self.rng = rng
        self.me = me
        self.opp = opp
...
def play_round(rng, pools, strat_names, first_actor, stats, pair_key):
    ...
    while True:
        ...
        resolved, ends_betting, is_raise_like = _legalize_and_apply(
            intent, p, pools, commits, bet_level, raises_used, call_amount, facing_bet)
...
def _legalize_and_apply(intent, p, pools, commits, bet_level, raises_used, call_amount, facing_bet):
    ...
    if intent == "bet":
        if facing_bet:
            ...
            amt, went_allin = pay(KNOBS["bet_amount"])
            new_level = commits[p]
            if went_allin and amt < KNOBS["bet_amount"]:
    ...
    if intent == "raise":
        ...
        if raises_used >= KNOBS["raise_cap"]:
            ...
            target = bet_level + KNOBS["raise_amount"]
...
def play_game(rng, strat_A, strat_B, first_actor, stats, pair_key):
    ...
    for rnd in range(1, KNOBS["max_rounds"] + 1):
        rounds_played = rnd
        play_round(rng, pools, strat_names, first, stats, pair_key)
...
def build_report(matrix, stats, elapsed, pair_count, out_dir):
    ...
    # --- summary.md ---
    lines = []
    lines.append("# 思考課金ポーカー v0-A モンテカルロ検証 結果サマリ\n")
    lines.append(f"- 実行日時(BASE_SEED基準): {BASE_SEED}")
    lines.append(f"- 総ペア数: {pair_count} (8戦略の重複組合せ, 対角含む)")
    ...
    lines.append("## 3. 覗き発動率 x ポットサイズ帯\n")
    lines.append("| strategy | " + " | ".join(band_names) + " |")
    lines.append("|---" * (len(band_names) + 1) + "|")
    for nm in names:
        cells = []
        for b in band_names:
            peeks, opps = stats.peek_by_band.get((nm, b), [0, 0])
