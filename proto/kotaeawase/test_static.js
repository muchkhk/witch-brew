...
console.log('版番号バッジがv0.9であること:');
{
  ok(/const VER = 'v0\.9';/.test(uiSrc), "const VER = 'v0.9'; であること");
}

console.log('開始ゲートの構造修正（presenceをハードゲートから外す / v0.9指示書対応）:');
{
  // 工程1: seatsClaimed()新設。seatsPresent()の既存の意味（定義）は書き換えていないこと
  ok(/seatsClaimed\(\)\s*\{/.test(uiSrc), 'seatsClaimed() が定義されていること');
  ok(/seatsPresent\(\)\s*\{\s*\n\s*const s = this\.roomInfo\.seats \|\| \{\}, pr = this\.roomInfo\.presence \|\| \{\};\s*\n\s*return PLAYERS\.filter\(\(p\) => s\[p\] && pr\[s\[p\]\.uid\] === true\)\.length;/.test(uiSrc),
    'seatsPresent() の定義（presence込みの人数を返す既存の意味）が変更されていないこと');

  // 工程2: presenceWarnings新設。文言が「開始してよい」と伝わるトーンであること
  ok(/presenceWarnings\(\)\s*\{/.test(uiSrc), 'presenceWarnings() が定義されていること');
  ok(/そのまま開始して構いません/.test(uiSrc), 'presence警告文言に「そのまま開始して構いません」という許可のトーンが含まれること');
  const renderWaitIdx2 = uiSrc.indexOf('renderWait() {');
  const renderWaitBody2 = renderWaitIdx2 >= 0 ? uiSrc.slice(renderWaitIdx2, renderWaitIdx2 + 2500) : '';
  ok(/this\.presenceWarnings\(\)/.test(renderWaitBody2), 'renderWait() が presenceWarnings() を参照すること（開始は妨げない警告として表示）');

  // 工程3: presenceハートビート。onHeartbeatErrorがDiagLogへ結線されていること
  ok(/function keepPresenceOpts\(/.test(uiSrc), 'keepPresenceOpts() が定義されていること');
  ok(/'presence:heartbeat-error'/.test(uiSrc), 'ハートビート失敗がDiagLogのイベント種別として記録されること');
  const keepPresenceCallCount = (uiSrc.match(/window\.Net\.keepPresence\([^)]*keepPresenceOpts\(\)\)/g) || []).length;
  ok(keepPresenceCallCount === 3, `window.Net.keepPresence() の全呼び出し(3箇所)がkeepPresenceOpts()を渡していること (実:${keepPresenceCallCount})`);
  ok(/heartbeatMs/.test(netSrc) && /setInterval/.test(netSrc), 'net.js の keepPresence() がハートビート(setInterval)を持つこと');
  ok(/onHeartbeatError/.test(netSrc), 'net.js の keepPresence() が onHeartbeatError フックを持つこと');

  // 工程4: startGame()早期returnの可視化
  const startGameIdx2 = uiSrc.indexOf('async startGame() {');
  const startGameBody2 = startGameIdx2 >= 0 ? uiSrc.slice(startGameIdx2, startGameIdx2 + 800) : '';
  ok(/開始処理を実行中です…/.test(startGameBody2) && /すでにホストとして起動済みです/.test(startGameBody2), 'startGame() の早期returnにflashError()の文言があること');

  // 触ってはいけないもの: net.jsのloadOrCreate/publish/resume判定ロジック本体は変更していないことの間接確認
  // （publish()の中核ロジック行が残っていること。工程5見送りによりonPublish自体も変更していない）
  ok(/self\.onPublish\(st\);/.test(netSrc), 'net.js の publish() が既存どおり onPublish(st) を呼ぶこと（工程5見送り・変更なし）');
}
